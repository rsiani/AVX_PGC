...
IN = read.xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx",
                sheet = "growth_assay",
                sep.names = " ") %>%
  select(c(bacteria, 
           `plant part`, 
...
effects = read.xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx",
...
copy = tDF[, 1:10] %>%
  pivot_longer(cols = 2:10,
               names_to = "Metric",
               values_to = "changeRatio") %>%
  group_by(bacteria) %>%
  summarise(mean(changeRatio))

## plot heatmap

superheat(tDF[, 2:10],
          scale = F,
          yr = copy$`mean(changeRatio)`,
          yr.axis.name = "Average Change",
          yr.plot.size = 0.2,
          ...
          legend.text.size = 15,
          smooth.heat = T,
          heat.pal = c("#264653","#2a9d8f","#e9c46a","#f4a261","#e76f51"),
          yr.cluster.col = c("#264653","#2a9d8f","#e9c46a","#f4a261","#e76f51"),
...
res.lda = read.xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx", 
                sheet = "res.lefse",
                colNames = T,
                rowNames = F,
                sep.names = " ") %>% 
...
ANI = readxl::read_xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx",
                            sheet = "ANI") %>%
  column_to_rownames("key")

## load metadata for the plotting

classes = readxl::read_xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx",
                            sheet  = "metadata")

## hierarchical clustering (default distance, euclidean)

res_hc = hclust(?dist(matrice),
                method = "ward.D2")
...
pangenome_info = readxl::read_xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx",
                                   sheet = "splits") %>%
...
DF0 = read.xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx",
                sheet = "summary",
                rowNames = F,
                colNames = T,
                sep.names = " ") %>% 
...
B = ggboxplot(DF0,
                ...
                xlab = "",
                ylab = "Logarithmic Genome Lenght",
                ggtheme = theme_ipsum(plot_margin = margin(0, 0, 0, 0),
                 axis_title_size = 15,
                 base_size = 15)) +
...
TAB = read.xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx", 
                sheet = "panMatrix",
                colNames = T,
                rowNames = T,
                sep.names = " ") %>% 
...
C = ggbarplot(cntrb,
              ...
              alpha = 0.66,
              rotate = T,
              palette = get_palette(c("#264653","#e9c46a","#e76f51"),3)) +
  theme_ipsum(plot_margin = margin(0, 0, 0, 0),
              base_size = 12.5,
              axis_title_size = 15) +
  theme(axis.title.y = element_blank())
...
DF3 = read.xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx",
...
DF4 = read.xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx",
                sheet = "curatedList") %>% 
  mutate(log2fold = log2(enrichment_score))
...
DF2 = read.xlsx("~/Desktop/AvxPGC/IN/res_AVX.xlsx", 
                        sheet = "antiSMASH",
                sep.names = " ") %>% 
